...
class Board(models.Model):
    ...
    def _compute_report_ids(self):
        for rec in self:
            rec.project_ids = (
                self.env["project.agile.board.report"]
    ...
    def declare_default(self):
        self.ensure_one()
        self.search(
            [("type", "=", self.type), ("is_default", "=", True)]
...
class Column(models.Model):
    ...
    board_id = fields.Many2one(
        comodel_name="project.agile.board", string="Board", ondelete="cascade"
    )
    ...
    min = fields.Integer(
        "Min", help="The minimum number of issues in this column"
    )

    max = fields.Integer(
        "Max", help="The maximum number of issues int this column"
    )
...
class ColumnStatus(models.Model):
    ...
    name = fields.Char(related="state_id.name", readonly=True)
    ...
    order = fields.Float(string="Order", required=False)
