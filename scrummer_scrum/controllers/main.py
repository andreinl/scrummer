...
class ScrummerController(main.ScrummerController):
    ...
    def sprint_start(self, sprint, start_date, end_date):
        sprint.write(
            {"state": "active", "start_date": start_date, "end_date": end_date}
        )
    ...
    def sprint_stop(self, sprint):
        # Mark sprint as done
        sprint.write(
            {"state": "completed", "actual_end_date": fields.Datetime.now()}
        )
    ...
    @http.route("/scrummer/web/data/sprint/create", type="json", auth="user")
    ...
    @http.route(
        [
            "/scrummer/web/data/active_sprints/"
            '<model("project.agile.board"):board>',
        ],
        type="json",
        auth="user",
    )
    def active_sprints(self, board, **options):
        ...
        board_data = {
            "board": {
                "name": board.name,
                "projects": {},
                "task_types": board.board_task_type_ids.ids,
            },
            "workflow": self.prepare_board_workflow(board),
            "active_sprints": {},
            "users": {},
            "ids": [],
        }
    ...
    def _prepare_sprint_task_filter(
        self, active_sprint, board_data, stages_in_board, board
    ):
        task_filter = [
        ...
        if board.board_task_type_ids:
            task_filter.append(
                ("type_id", "in", board.board_task_type_ids.ids)
            )

        return task_filter

    def prepare_task(self, task):
        task_data = super(ScrummerController, self).prepare_task(task)
        task_data.update(
