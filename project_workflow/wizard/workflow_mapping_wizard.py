...
class WorkflowMappingWizard(models.TransientModel):
    ...
    from_id = fields.Many2one(comodel_name="project.workflow", string="From")

    to_id = fields.Many2one(comodel_name="project.workflow", string="To",)
    ...
    switch = fields.Boolean(string="Just Switch", default=False,)
...
class WorkflowMappingWizardLine(models.TransientModel):
    ...
    task_count = fields.Integer(
        string="Task Count", related="from_id.task_count", readonly=True,
    )
    ...
    to_stage_id = fields.Selection(
        selection="_compute_to_stage_id", string="To Stage",
    )

    _sql_constraints = [
        (
            "unique_stages",
            "UNIQUE(wizard_id, from_id, to_id)",
...
class WorkflowMappingWizardStage(models.TransientModel):
    ...
    type = fields.Selection(
        selection=[("from", "From"), ("to", "To")], string="Type",
    )
    ...
    stage_id = fields.Many2one(
        comodel_name="project.task.type", string="Stage", required=False,
    )

    name = fields.Char(string="Name", related="stage_id.name", readonly=True,)
