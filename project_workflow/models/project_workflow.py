...
class Workflow(models.Model):
    ...
    original_id = fields.Many2one(
        comodel_name="project.workflow", string="Origin", ondelete="cascade",
    )

    original_name = fields.Char(
        string="Original", related="original_id.name", readonly=True,
    )
    ...
    edit_count = fields.Integer(
        string="Edit Count", compute="_compute_edit_count",
    )
...
class WorkflowState(models.Model):
    ...
    name = fields.Char(string="Name", related="stage_id.name", requried=True,)

    description = fields.Text(
        string="Description", related="stage_id.description", requried=True,
    )
    ...
    xpos = fields.Integer(string="X", default=50, copy=True,)

    ypos = fields.Integer(string="Y", default=50, copy=True,)

    sequence = fields.Integer(string="Sequence", default=0,)

    kanban_sequence = fields.Integer(string="Kanban Sequence", default=0,)
...
class WorkflowTransition(models.Model):
    ...
    user_confirmation = fields.Boolean(
        string="User Confirmation?", default=False
    )

    _sql_constraints = [
        (
            "unique_src_dst",
            "UNIQUE(workflow_id,src_id,dst_id)",
