...
class ProjectWorkflowPublisher(models.AbstractModel):
    ...
    def publish(self, old, new, mappings=None, project_id=None, switch=False):
        if not new:
            raise exceptions.ValidationError(
                _("You have to provide the new workflow!")
            )
    ...
    def _do_publish(self, old, new, project_id=None, switch=False):
        if switch:
            ...
            if project_id:
                project_id.write(
                    {"workflow_id": new.id, "type_ids": stage_ids}
                )
            else:
    ...
    def _build_mapping_wizard(
        self, old, new, result, project_id=None, switch=False
    ):
        # Create Wizard
        ...
        for stage in result["stages"]:
            wstage = self.env[
                "project.workflow.stage.mapping.wizard.stage"
